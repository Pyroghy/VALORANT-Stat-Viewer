const { getPresences, getPlayerInfo } = require('./Presences');
const { getPlayerRank, getPlayerStats } = require('./Stats');
const fetch = require('../modules/fetch');
const self = process.app;

async function getMatchId(gameState) {
    return fetch(`https://glz-${self.region}-1.${self.region}.a.pvp.net/${gameState}/v1/players/${self.puuid}`).then(data => data.MatchID);
}
...
module.exports = { getMatchId, getPlayers }
